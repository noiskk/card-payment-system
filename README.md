...
시작은 우리FISA 팀 프로젝트다. 이후 개인 저장소에서 정합성 부분을 단독으로 보강했다.

| 영역 | 팀 프로젝트 | 개인 고도화 |
|---|---|---|
| 승인 경로 | POS→VAN→FDS→Payment→Bank 동기 호출 | (유지) |
| 예외 처리 | 서비스마다 제각각 | Domain/Business/System 계층 통일, "전송=200·결과=응답코드" 규약 |
| 멱등성 | 없음 (재시도 = 이중결제) | 예약-후-실행 멱등성 + STAN 멱등키 전 구간 전파 |
| 원장 | payment의 로컬 테이블, 가변 | **독립 서비스로 분리, INSERT-only 불변** |
| 카드사 구조 | payment + FDS 2개, FDS가 승인 흐름까지 담당 | **게이트웨이·승인·원장·FDS로 분해, 승인이 오케스트레이션** |
| 서비스 탐색 | URL 하드코딩 | Eureka 서비스 이름 기반 (내부에 한해) |
| 정합성 복구 | 없음 | **보상 트랜잭션 + 망취소 + 은행 취소 API(멱등)** |
| 정산/배치 | 없음 | **Spring Batch — 망취소 대사, 가맹점별 정산** |
| 테스트 | 일부 깨짐 | 64개 (정합성 로직 중심) |

담당 범위: 팀 단계는 card-service(FDS + Payment), 개인 단계는 전 서비스.
...
| DB 설정이 통째로 무시됨 | `application.yml`에 `spring:` 키 중복. YAML은 뒤엣것이 앞엣것을 덮어씀 | 기존 블록에 병합 |
| 클론하면 `./gradlew` 실행 불가 | `.gitignore`에서 `*.jar`가 wrapper 예외 규칙보다 뒤에 있어 예외가 무효화 | 예외 규칙을 파일 끝으로 |
