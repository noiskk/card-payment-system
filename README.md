...
flowchart TB
    ...
    subgraph issuer2["비카드 (라우팅 대상 · 미구현)"]
        GW2["게이트웨이<br/>:9100"]
    end
...
비카드는 라우팅 테이블에만 있고 실제 서버는 만들지 않았다. 라우팅이 정말로 대상을 바꾸는지 확인하는 용도다.
...
**응답코드**: `00` 승인 · `14` 카드상태 · `15` 연동 카드사 없음 · `51` 잔액/한도부족 · `61` 1회한도초과 · `94` 중복의심 · `96` 시스템오류

---

## 핵심 설계 결정

### 1. VAN은 카드번호로 보낼 카드사를 고른다

가맹점이 VAN 한 곳만 연동하면 모든 카드사에 닿는다는 것이 VAN의 존재 이유다. 그러려면 카드번호 앞 6자리(BIN)로 발급 카드사를 판별해야 한다.

라우팅 테이블은 코드가 아니라 설정에 뒀다. 카드사 추가·BIN 대역 변경은 일상적인 일이라 그때마다 배포할 수 없다.
BIN이 6자리에서 8자리로 세분화되면 대역이 겹치므로 **최장 일치**로 대상을 정한다 — 등록 순서에 결과가 좌우되면 안 된다.

**보낼 카드사를 못 찾으면 중계하지 않는다.** 아무 카드사에나 보내면 남의 거래 전문이 그 카드사에 도달한다.
이때 응답코드는 시스템 오류(`96`)가 아니라 `15`(No such issuer)다. VAN이 고장난 게 아니라 연동 대상이 아니라는 뜻이어서, 가맹점이 원인을 구분할 수 있어야 한다.

연동 중단은 설정에서 지우지 않고 `enabled: false`로 표시한다. 지워버리면 그 BIN이 미등록과 구분되지 않아 원인 추적이 막힌다.

### 2. 타임아웃 시 취소를 보내지 않는다
...
| VAN 중계 모니터 | http://localhost:7070 | 카드사 라우팅 테이블, 중계 내역, 라우팅된 카드사, TCP/HTTP 채널, 멱등키 |
...
4. **BIN 라우팅** — `6222221111111111`로 결제하면 비카드(:9100)로 라우팅되어 연동 실패(`96`)가 나고, `7777771111111111`은 라우팅 테이블에 없어 카드사에 닿기도 전에 `15`로 거절된다. VAN 화면의 카드사 칸에서 어디로 갔는지 확인할 수 있다
